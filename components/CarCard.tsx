...
const CarCard = (car: Car) => {
  ...
  const [imageUrl, setImageUrl] = useState(CAR_PLACEHOLDER)

  useEffect(() => {
    if (car.image) {
      setImageUrl('http://localhost:8080/images/' + car.image)
    }
  }, [car])
  ...
  return (
    <div className="border border-black">
      ...
      <div className="p-4">
        <h2>{car.name}</h2>
        <div className="my-4">
          <p><span className="font-bold">{rupiah(car.dayRate)}</span> / hari</p>
          <p>⭐ {car.rating}</p>
        </div>
      </div>
    </div>
